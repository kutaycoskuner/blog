...
const $$Index = createComponent(async ($$result, $$props, $$slots) => {
  const COLLECTION_NAME = "collections/lecture_notes-2025fall-analysis";
  ...
  return renderTemplate`${renderComponent($$result, "Layout", $$Layout, { "data-astro-cid-4jybnszh": true }, { "default": async ($$result2) => renderTemplate(_a || (_a = __template([" ", '<div class="main markdown" data-astro-cid-4jybnszh> ', ' </div> <div class="toc-container no-select" data-astro-cid-4jybnszh> ', ' </div> <script>\n        let activeArticle = null;\n\n        const articles = document.querySelectorAll("article");\n        const tocSections = document.querySelectorAll(".toc-sections");\n        const tocTitles = document.querySelectorAll(".toc-title");\n\n        // Show/hide TOC sections on mouse enter\n        articles.forEach((article) => {\n            article.addEventListener("mouseenter", () => {\n                activeArticle = article.id;\n                tocSections.forEach((section) => {\n                    section.style.display =\n                        section.dataset.slug === activeArticle\n                            ? "block"\n                            : "none";\n                });\n            });\n        });\n\n        // Click TOC title to scroll & set active article\n        tocTitles.forEach((title) => {\n            title.addEventListener("click", () => {\n                const slug = title.parentElement.dataset.slug;\n                const targetArticle = document.getElementById(slug);\n                if (!targetArticle) return;\n\n                // scroll to top of the article\n                targetArticle.scrollIntoView({ behavior: "smooth" });\n\n                // set active article\n                activeArticle = slug;\n                tocSections.forEach((section) => {\n                    section.style.display =\n                        section.dataset.slug === activeArticle\n                            ? "block"\n                            : "none";\n                });\n            });\n        });\n\n        // Smooth scroll for TOC links\n        document.querySelectorAll(".toc-content").forEach((link) => {\n            link.addEventListener("click", (e) => {\n                e.preventDefault();\n                const target = document.getElementById(\n                    link.getAttribute("href").substring(1)\n                );\n                target?.scrollIntoView({ behavior: "smooth" });\n            });\n        });\n    <\/script> '])), maybeRenderHead(), await Promise.all(
    posts.map(async (post) => {
      const { Content } = await post.render();
      const postData = post.data;
      if (!postData.visibility) return null;
      return renderTemplate`<article class="article content"${addAttribute(post.slug, "id")} data-astro-cid-4jybnszh> <div class="article-meta" data-astro-cid-4jybnszh> <h1 data-astro-cid-4jybnszh>${postData.title}</h1> <span data-astro-cid-4jybnszh>
Published: ${formatDate(postData.created)} </span> <span data-astro-cid-4jybnszh>
Revision: ${formatDate(postData.updated)}, v
${postData.revision} </span> <span data-astro-cid-4jybnszh>
Language:${" "} ${getLanguageDescription(postData.language)} </span> </div> ${renderComponent($$result2, "Content", Content, { "data-astro-cid-4jybnszh": true })} </article>`;
    })
  ), postTocs.map(({ postData, toc, slug }) => {
    if (!postData.visibility) return null;
    return renderTemplate`<div class="toc-article"${addAttribute(slug, "data-slug")} data-astro-cid-4jybnszh> <div class="toc-title" data-astro-cid-4jybnszh> <span style="color: var(--col)" data-astro-cid-4jybnszh> <b data-astro-cid-4jybnszh>${postData.title}</b> </span> </div> <div class="toc-sections"${addAttribute(slug, "data-slug")} data-astro-cid-4jybnszh> ${toc.map((link) => renderTemplate`<span${addAttribute(`toc-item toc-level-${link.depth}`, "class")}${addAttribute(`margin-left: ${link.depth === 2 ? "0.5em" : link.depth === 3 ? "1.5em" : link.depth >= 4 ? "2.5em" : "0"};`, "style")} data-astro-cid-4jybnszh> <a class="toc-content"${addAttribute(`#${link.slug}`, "href")} data-astro-cid-4jybnszh> ${link.text} </a> <br data-astro-cid-4jybnszh> </span>`)} </div> </div>`;
  })) })} `;
}, "C:/Users/kutay/OneDrive/Documents/GitHub/blog/src/pages/collections/2025fall-analysis/index.astro", void 0);

const $$file = "C:/Users/kutay/OneDrive/Documents/GitHub/blog/src/pages/collections/2025fall-analysis/index.astro";
const $$url = "/blog/collections/2025fall-analysis";
